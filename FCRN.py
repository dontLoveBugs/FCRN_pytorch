...
class UpProject(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('out2_1234 = ', out2_1234.size())
...
class UpModules(nn.Module):

    def __init__(self, batch_size):
        super(UpModules, self).__init__()
        self.batch_size = batch_size
        self.relu = nn.ReLU(inplace=True)
        # Up-Conv layers
...
class FCRN(nn.Module):
    ...
    def __init__(self, batch_size, pre_trained = False):
        ...
        self.features = FeatureExtractor(pre_trained)
        self.upSample = UpModules(batch_size=batch_size)
...
def FCRN_pretrain(batch_size = 16):
    model = FCRN(batch_size = batch_size, pre_trained=True)
    ...
    model_features_dict = model.features.state_dict()

    # 选择预训练模型加载的参数
    pretrain_dict = {k:v for k, v in pretrain_dict.items() if k in model_features_dict}
    # print('pretrain_dict in model = ')
    # for k, v in pretrain_dict.items():
    #     print(k)

    # 更新model_dict权重
    model_features_dict.update(pretrain_dict)
    model.features.load_state_dict(pretrain_dict)
    return model

if __name__ == '__main__':
    model = FCRN_pretrain(batch_size=16)
    print('model 构建完成！')
